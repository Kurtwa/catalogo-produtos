...
SOURCE_PDF = "HY&HM Series SYT.pdf"
...
OUT_DIR = BASE / "ocr-hy-hm"
CODE_RE = re.compile(r"(?<![A-Z0-9-])((?:HY|HM)\d{1,3}[A-Z]?)(?![A-Z0-9-])")
...
def split_name_and_specs(value: str) -> tuple[str, str]:
    markers = ["N.W", "N W", "DIM", "CTN", "WS:", "Weight Stack"]
...
def visual_description(name: str) -> str:
    text = name.lower()
    if any(word in text for word in ["bench", "roman chair"]):
        return "Banco ou apoio de treino HY/HM, estrutura metálica com estofado para exercícios livres ou guiados."
    if any(word in text for word in ["rack", "smith", "dumbbell", "barbell", "storage"]):
        return "Rack ou suporte HY/HM para barras, halteres ou treino livre, com estrutura metálica robusta."
    if any(word in text for word in ["row", "pull", "pulldown", "back", "dorsal"]):
        return "Máquina de costas/remada HY/HM, com estação guiada, braços ou polias e apoios acolchoados."
    if any(word in text for word in ["chest", "pec", "fly", "press"]):
        return "Máquina de peitoral/press HY/HM, com assento e alavancas para empurrar ou abrir."
    if any(word in text for word in ["shoulder", "delt", "lateral"]):
        return "Máquina de ombro/deltoide HY/HM, com assento e braços de movimento guiado."
    if any(word in text for word in ["leg", "thigh", "glute", "hip", "calf", "squat", "abductor"]):
        return "Máquina de pernas/glúteos HY/HM, com plataforma, roletes ou apoios para membros inferiores."
    if any(word in text for word in ["biceps", "triceps", "arm", "curl", "wrist"]):
        return "Máquina de braços HY/HM, com apoio acolchoado e estação para bíceps, tríceps ou punho."
    if any(word in text for word in ["abdominal", "torso", "rotation", "crunch", "waist"]):
        return "Máquina de abdômen/core HY/HM, com assento e apoios para flexão ou rotação."
    if any(word in text for word in ["multi", "functional", "trainer"]):
        return "Estação multifuncional HY/HM, estrutura alta com múltiplos pontos de exercício."
    return "Equipamento de musculação HY/HM, associado à foto do mesmo bloco visual no PDF."


def tags_for(name: str) -> list[str]:
    text = name.lower()
    tags = ["syt", "hy&hm", "musculacao", "academia"]
    rules = [
        (["bench"], ["banco"]),
        ...
        (["dumbbell", "barbell"], ["halteres", "barra", "suporte"]),
        (["row", "back", "dorsal"], ["costas", "remada"]),
        (["pull", "pulldown"], ["costas", "puxada"]),
        (["chest", "press"], ["peitoral", "supino"]),
        (["pec", "fly"], ["peitoral", "voador"]),
        (["shoulder", "delt", "lateral"], ["ombro", "desenvolvimento"]),
        (["leg press"], ["leg press", "pernas"]),
        (["leg", "thigh", "glute", "hip", "calf", "squat", "abductor"], ["pernas"]),
        (["biceps", "curl"], ["bracos", "biceps"]),
        (["triceps"], ["bracos", "triceps"]),
        (["wrist"], ["punho", "antebraco"]),
        (["abdominal", "torso", "rotation", "crunch", "waist"], ["abdomen", "core"]),
        (["multi", "functional", "trainer"], ["funcional"]),
    ]
...
def main() -> None:
    ...
    markdown_lines = [
        "# HY&HM Series SYT - tabela validada",
        "",
        "Resumo: cruzamento visual/OCR concluído, todas as 24 páginas do PDF HY&HM foram processadas e cada linha foi associada à imagem pelo código do produto.",
    ...
    markdown_lines.append("")
    markdown_lines.append("Processamento do PDF 7 concluído. Por favor, envie o próximo arquivo!")
    (OUT_DIR / "HY_HM_Series_SYT_validado.md").write_text("\n".join(markdown_lines), encoding="utf-8")
    write_csv(OUT_DIR / "hy_hm_apply_report.csv", report_rows)
