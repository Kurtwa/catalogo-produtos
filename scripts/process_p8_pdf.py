...
SOURCE_PDF = "P8 series price list.pdf"
...
def visual_description(name: str) -> str:
    text = name.lower()
    if any(word in text for word in ["bench", "power cage", "rack"]):
        return "Banco, rack ou estrutura livre da linha P8, com armação metálica e área de apoio para treino."
    if any(word in text for word in ["row", "pull", "pulldown", "lat"]):
        return "Máquina de costas/puxada da linha P8, com torre guiada, polias ou estação de remada."
    if any(word in text for word in ["leg", "thigh", "hip", "glute", "calf", "squat"]):
        return "Máquina de pernas/glúteos da linha P8, com roletes, plataforma ou apoios inferiores."
    if any(word in text for word in ["chest", "pec", "fly"]):
        return "Máquina de peitoral/press da linha P8, com assento e braços articulados."
    if any(word in text for word in ["shoulder", "lateral"]):
        return "Máquina de ombro da linha P8, com assento e alavancas para elevação ou desenvolvimento."
    if any(word in text for word in ["biceps", "triceps", "dip", "curl"]):
        return "Máquina de braços da linha P8, com apoio e estação para bíceps, tríceps ou dips."
    if any(word in text for word in ["abdominal", "torso", "lower back", "roman"]):
        return "Máquina de core/lombar da linha P8, com assento e apoios para abdômen ou extensão."
    return "Equipamento de musculação da linha P8, associado à foto do mesmo bloco visual no PDF."


def tags_for(name: str) -> list[str]:
    text = name.lower()
    tags = ["syt", "p8", "musculacao", "academia"]
    rules = [
        (["bench"], ["banco"]),
        (["flat bench"], ["banco reto"]),
        (["decline"], ["declinado"]),
        (["incline"], ["inclinado"]),
        (["rack", "power cage"], ["rack", "suporte"]),
        (["row"], ["costas", "remada"]),
        ...
        (["leg", "thigh", "hip", "glute", "calf", "squat"], ["pernas"]),
        (["biceps", "curl"], ["bracos", "biceps"]),
        (["triceps", "dip"], ["bracos", "triceps"]),
        (["abdominal", "torso", "lower back", "roman"], ["abdomen", "core"]),
    ]
...
def main() -> None:
    ...
    markdown_lines = [
        "# P8 series price list - tabela validada",
        "",
        "Resumo: cruzamento visual/OCR concluído, todas as 11 páginas do PDF P8 foram processadas e cada linha foi associada à imagem pelo código do produto.",
    ...
    markdown_lines.append("")
    markdown_lines.append("Processamento do PDF 13 concluído. Por favor, envie o próximo arquivo!")
    (OUT_DIR / "P8_series_price_list_validado.md").write_text("\n".join(markdown_lines), encoding="utf-8")
    write_csv(OUT_DIR / "p8_apply_report.csv", report_rows)
