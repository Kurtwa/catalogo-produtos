...
SOURCE_PDF = "SQ series price list.pdf"
...
OUT_DIR = BASE / "ocr-sq"
CODE_RE = re.compile(r"(?<![A-Z0-9-])(SQ\d{4}[A-Z]?)(?![A-Z0-9-])")
...
def visual_description(name: str) -> str:
    text = name.lower()
    if any(word in text for word in ["smith", "cage", "rack", "barbell"]):
        return "Rack, smith ou estrutura livre da linha SQ, com armação metálica para barras e treino multifuncional."
    if any(word in text for word in ["bench"]):
        return "Banco de treino da linha SQ, estrutura metálica com estofado para exercícios livres."
    if any(word in text for word in ["trainer", "station", "functional"]):
        return "Estação multifuncional da linha SQ, com cabos, polias ou múltiplos pontos de exercício."
    if any(word in text for word in ["leg", "hip", "glute", "squat", "thrust"]):
        return "Equipamento de pernas/glúteos da linha SQ, com plataforma, suporte ou apoios inferiores."
    if any(word in text for word in ["chest", "fly"]):
        return "Equipamento de peitoral da linha SQ, com apoio e braços articulados."
    if any(word in text for word in ["shoulder", "high pull"]):
        return "Equipamento de ombro/costas da linha SQ, com apoio e alavanca ou polia."
    return "Equipamento de musculação da linha SQ, associado à foto do mesmo bloco visual no PDF."


def tags_for(name: str) -> list[str]:
    text = name.lower()
    tags = ["syt", "sq", "musculacao", "academia"]
    rules = [
        ...
        (["incline"], ["inclinado"]),
        (["flat"], ["banco reto"]),
        (["functional", "trainer", "station"], ["funcional", "polia"]),
        (["leg press"], ["leg press", "pernas"]),
        (["leg", "hip", "glute", "squat", "thrust"], ["pernas", "gluteo"]),
        (["chest", "fly"], ["peitoral", "supino"]),
        (["shoulder"], ["ombro", "desenvolvimento"]),
        (["high pull"], ["costas", "puxada"]),
        (["barbell"], ["barra", "suporte"]),
    ]
...
def image_by_order() -> dict[tuple[int, int], str]:
    image_dir = BASE / "product-images" / "sq-series-price-list"
    mapping = {}
    for image in sorted(image_dir.glob("p*_img*.jpg")):
        ...
        mapping[(int(match.group(1)), int(match.group(2)))] = (
            f"./imports/syt-05-2026/product-images/sq-series-price-list/{image.name}"
        )
    return mapping
...
def main() -> None:
    ...
    images = image_by_order()
    page_counts: dict[int, int] = {}
    supplier_id = stable_uuid("syt-05-2026-supplier")
    file_id = stable_uuid(f"syt-05-2026-file-{SOURCE_PDF}")

    markdown_lines = [
        "# SQ series price list - tabela validada",
        "",
        "Resumo: cruzamento visual/OCR concluído, todas as 7 páginas do PDF SQ foram processadas e cada linha foi associada à imagem pela ordem visual da página.",
    ...
    markdown_lines.append("")
    markdown_lines.append("Processamento do PDF 14 concluído. Por favor, envie o próximo arquivo!")
    (OUT_DIR / "SQ_series_price_list_validado.md").write_text("\n".join(markdown_lines), encoding="utf-8")
    write_csv(OUT_DIR / "sq_apply_report.csv", report_rows)
