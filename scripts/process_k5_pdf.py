...
SOURCE_PDF = "K5 series price list.pdf"
...
OUT_DIR = BASE / "ocr-k5"
CODE_RE = re.compile(r"(?<![A-Z0-9-])(K5\d{2}[A-Z]?)(?![A-Z0-9-])")
...
def clean(value: str) -> str:
    ...
    value = value.replace("Seated/Standin g", "Seated/Standing")
    return value.strip(" :-：+")
...
def visual_description(name: str) -> str:
    text = name.lower()
    if any(word in text for word in ["row", "smith"]):
        return "Máquina de costas/remada da linha K5, com estrutura guiada e apoios acolchoados."
    if any(word in text for word in ["chest", "fly"]):
        return "Máquina de peitoral/voador da linha K5, com banco e braços articulados."
    if any(word in text for word in ["shoulder", "lateral"]):
        return "Máquina de ombro/lateral da linha K5, com assento e alavancas de movimento guiado."
    if any(word in text for word in ["leg", "calf", "tibialis", "hip", "hack", "thigh"]):
        return "Máquina de pernas/glúteos da linha K5, com plataforma, roletes ou apoios para membros inferiores."
    if any(word in text for word in ["bicep", "tricep", "curl", "pushdown"]):
        return "Máquina de braços da linha K5, com apoio ou estação para bíceps/tríceps."
    if "hyper" in text:
        return "Máquina de extensão lombar da linha K5, com apoio acolchoado para tronco/quadril."
    return "Equipamento de musculação da linha K5, associado à foto do mesmo bloco visual no PDF."


def tags_for(name: str) -> list[str]:
    text = name.lower()
    tags = ["syt", "k5", "musculacao", "academia"]
    rules = [
        (["row"], ["costas", "remada"]),
        (["smith"], ["smith", "agachamento", "anilhada", "remada", "costas"]),
        (["chest"], ["peitoral", "supino"]),
        (["fly"], ["peitoral", "voador"]),
        (["shoulder", "lateral"], ["ombro", "desenvolvimento"]),
        (["leg", "calf", "tibialis", "hip", "hack", "thigh"], ["pernas"]),
        (["hack"], ["agachamento"]),
        (["bicep", "curl"], ["bracos", "biceps"]),
        (["tricep", "pushdown"], ["bracos", "triceps"]),
        (["hyper"], ["lombar", "core"]),
    ]
...
def main() -> None:
    ...
    markdown_lines = [
        "# K5 series price list - tabela validada",
        "",
        "Resumo: cruzamento visual/OCR concluído, todas as 2 páginas do PDF K5 foram processadas e cada linha foi associada à imagem pelo código do produto.",
    ...
    markdown_lines.append("")
    markdown_lines.append("Processamento do PDF 10 concluído. Por favor, envie o próximo arquivo!")
    (OUT_DIR / "K5_series_price_list_validado.md").write_text("\n".join(markdown_lines), encoding="utf-8")
    write_csv(OUT_DIR / "k5_apply_report.csv", report_rows)
