...
SOURCE_PDF = "K6 Series price list.pdf"
...
OUT_DIR = BASE / "ocr-k6"
CODE_RE = re.compile(r"^\s*\d+\s+(K6\d{2}[A-Z]?)(?![A-Z0-9-])", re.MULTILINE)
PRICE_RE = re.compile(r"\$\s*([0-9][0-9,.]*)")
SIZE_RE = re.compile(r"Assembly\s*Size\s*:\s*([0-9\s*]+mm)", re.IGNORECASE)
...
def visual_description(name: str) -> str:
    text = name.lower()
    if any(word in text for word in ["row", "pull", "pulldown"]):
        return "Máquina de costas/puxada da linha K6, com torre guiada, polias ou estação de remada."
    if any(word in text for word in ["leg", "calf", "abductor", "adductor", "kick"]):
        return "Máquina de pernas/glúteos da linha K6, com roletes, plataforma ou apoios inferiores."
    if any(word in text for word in ["chest", "pec", "fly"]):
        return "Máquina de peitoral/press da linha K6, com assento e braços articulados."
    if any(word in text for word in ["shoulder", "lateral"]):
        return "Máquina de ombro da linha K6, com assento e alavancas para elevação ou desenvolvimento."
    if any(word in text for word in ["biceps", "triceps", "curl", "dip"]):
        return "Máquina de braços da linha K6, com apoio e estação para bíceps, tríceps ou dips."
    if any(word in text for word in ["abdominal", "torso", "back extension"]):
        return "Máquina de core/lombar da linha K6, com assento e apoios para abdômen ou extensão."
    return "Equipamento de musculação da linha K6, associado à foto do mesmo bloco visual no PDF."


def tags_for(name: str) -> list[str]:
    text = name.lower()
    tags = ["syt", "k6", "musculacao", "academia"]
    rules = [
        (["row"], ["costas", "remada"]),
        ...
        (["leg", "calf", "abductor", "adductor", "kick"], ["pernas"]),
        (["biceps", "curl"], ["bracos", "biceps"]),
        (["triceps", "dip"], ["bracos", "triceps"]),
        (["abdominal", "torso", "back extension"], ["abdomen", "core"]),
    ]
...
def main() -> None:
    ...
    markdown_lines = [
        "# K6 Series price list - tabela validada",
        "",
        "Resumo: cruzamento visual/OCR concluído, todas as 8 páginas do PDF K6 foram processadas e cada linha foi associada à imagem pelo código do produto.",
    ...
    markdown_lines.append("")
    markdown_lines.append("Processamento do PDF 11 concluído. Por favor, envie o próximo arquivo!")
    (OUT_DIR / "K6_Series_price_list_validado.md").write_text("\n".join(markdown_lines), encoding="utf-8")
    write_csv(OUT_DIR / "k6_apply_report.csv", report_rows)
