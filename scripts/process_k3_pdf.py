...
SOURCE_PDF = "K3 series price list.pdf"
...
OUT_DIR = BASE / "ocr-k3"
CODE_RE = re.compile(r"(?<![A-Z0-9-])(K3\d{2}[A-Z]?)(?![A-Z0-9-])")
...
def visual_description(name: str) -> str:
    text = name.lower()
    if any(word in text for word in ["row", "pull", "back"]):
        return "Máquina de costas/remada da linha K3, com estação guiada e apoios acolchoados."
    if any(word in text for word in ["chest", "press"]):
        return "Máquina de peitoral/press da linha K3, com assento e braços articulados."
    if "shoulder" in text:
        return "Máquina de ombro da linha K3, com assento e alavancas para desenvolvimento."
    if any(word in text for word in ["leg", "calf", "kick"]):
        return "Máquina de pernas/glúteos da linha K3, com roletes, plataforma ou apoios inferiores."
    if any(word in text for word in ["biceps", "triceps", "curl"]):
        return "Máquina de braços da linha K3, com apoio acolchoado e estação para bíceps ou tríceps."
    return "Equipamento de musculação da linha K3, associado à foto do mesmo bloco visual no PDF."


def tags_for(name: str) -> list[str]:
    text = name.lower()
    tags = ["syt", "k3", "musculacao", "academia"]
    rules = [
...
def main() -> None:
    ...
    markdown_lines = [
        "# K3 series price list - tabela validada",
        "",
        "Resumo: cruzamento visual/OCR concluído, todas as 2 páginas do PDF K3 foram processadas e cada linha foi associada à imagem pelo código do produto.",
    ...
    markdown_lines.append("")
    markdown_lines.append("Processamento do PDF 9 concluído. Por favor, envie o próximo arquivo!")
    (OUT_DIR / "K3_series_price_list_validado.md").write_text("\n".join(markdown_lines), encoding="utf-8")
    write_csv(OUT_DIR / "k3_apply_report.csv", report_rows)
